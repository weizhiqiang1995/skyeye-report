...
layui.config({
    ...
    showGrid({
	 	id: "showForm",
	 	url: reqBasePath + "reportbgimage001",
	 	params: {},
	 	pagination: true,
	 	pagesize: 18,
	 	...
	 	options: {'click .del':function(index, row){
				layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
					layer.close(index);
		            AjaxPostUtil.request({url:reqBasePath + "reportbgimage003", params:{rowId: row.id}, type:'json', method: "DELETE", callback:function(json){
