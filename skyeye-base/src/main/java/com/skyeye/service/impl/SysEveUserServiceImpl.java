		...
		if(userMation == null){
			...
			if(password.equals(userMation.get("password").toString())){
				...
				if(STATE.SYS_USER_LOCK_STATE_ISLOCK.getState() == userLock){
					...
					jedisClient.set("userMation:" + userMation.get("id").toString(), JSONUtil.toJsonStr(userMation));
					jedisClient.set("deskTopsMation:" + userMation.get("id").toString(), JSONUtil.toJsonStr(deskTops));
					jedisClient.set("allMenuMation:" + userMation.get("id").toString(), JSONUtil.toJsonStr(allMenu));
					jedisClient.set("authPointsMation:" + userMation.get("id").toString(), JSONUtil.toJsonStr(authPoints));
